...
public:
  ...
  // Complejidad: O(N)
  int minByIndex(int sortedIndex){ //esta funcion nos da el indice del minimo valor dejando fuera 
                                  // valores, el numero de valor que le pases, es lo que le va a restar al indice 
    int min_index = -1; // O(1)
    T min_val = peekMax(); // O(1)
    int n = sizeQ(); // O(1)
    for (int i=0; i<n; i++){ // O(N)
      T curr = dequeue(); // O(1)
      
        // agregamos la condicion i <= sortedIndex por que no queremos modificar los elementos 
        // que van despues de nuestro indice especificado, por que ya estan en orden
        if (curr <= min_val && i <= sortedIndex){ // O(1)
          min_index = i; // O(1)
          min_val = curr; // O(1)
        }
      enqueue(curr); // O(1)
  ...
  // Complejidad: O(N)
  void insertMinAtras(int minByIndex){
    T min_val; // O(1)
    int n = sizeQ(); // O(1)
    for (int i = 0; i < n; i++){ // O(N)
      T curr = dequeue(); // O(1)
